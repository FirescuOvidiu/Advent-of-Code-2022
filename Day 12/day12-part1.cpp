...
int findSteps(const vector<string> & m, int startX, int startY, int endX, int endY)
{
    ...
    for (int i = 0; i < sizeQ; i++)
    {
      ...
      for (int k = 0; k < 4; k++)
      {
        ...
        if (checkInMap(newX, newY, m.size(), m[0].size()) &&
            (m[newX][newY] - m[currX][currY] <= 1) && v[newX][newY] == false)
        {
          {
            q.push({ newX, newY });
            v[newX][newY] = true;
...
int main()
{
  ...
  out << findSteps(m, startX, startY, endX, endY);
